...
def get_pr_diff(base_branch: str, head_branch: str):
    """
    Получить diff между двумя ветками для PR review.
    ...
    Returns:
        JSON с diff между ветками
    """
    logger.info(f"Getting PR diff: {base_branch}...{head_branch}")

    # Используем triple-dot notation для сравнения веток
    # base...head показывает изменения в head с момента ответвления от base
    command = ['git', 'diff', f'{base_branch}...{head_branch}']
    ...
    # Информация о размере diff
    lines_count = len(diff.split('\n'))
    chars_count = len(diff)

    # Ограничить размер для больших PR (50KB)
    max_chars = 50000
    truncated = False
...
async def list_tools() -> list[Tool]:
    ...
    tools = [
        ...
        Tool(
            name="get-pr-diff",
            description="Получить diff между двумя ветками для PR review",
            inputSchema={
                "type": "object",
                "properties": {
                    "base_branch": {
                    ...
                    "head_branch": {
                        "type": "string",
                        "description": "Ветка с изменениями (например, 'feature/new-feature')"
                    }
                },
